...
class SimpleTodos extends Component {
    ...
    handleAddTodo = () => {
        ...
        const newTodos = Array.from({lenght: newTodoCount}, (_, i) => ({
            id: Date.now() + i,
            title: newTodoTitle,
            completed: false,
        }))
    ...
    deleteTodo = id => {
        this.setState(prevState => {
            const updateTodoList = prevState.todosList.map(todo =>
             todo.id = id ? {...todo, completed: !todo.completed} : todo,
             )
             return {todosList: updateTodoList}
        })
    }
